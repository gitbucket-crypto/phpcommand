...
function createBash()
{
    $bash="#!/bin/bash
        ...
        ./soc
    ";
    $fp = fopen( getcwd().DIRECTORY_SEPARATOR. 'soc.sh' ,'w+');
    fwrite($fp,  $bash);
    fclose($fp);
    shell_exec(bash.' && chmod a+x soc.sh');

    unset($bash);
    $bash="#!/bin/bash
       echo 'executing smartregua'
       ./smartregua
    ";

    $fp = fopen( getcwd().DIRECTORY_SEPARATOR. 'smartregua.sh' ,'w+');
    fwrite($fp,  $bash);
    fclose($fp);
    shell_exec(bash.' && chmod a+x smartregua.sh');
    ...
    $fp = fopen( getcwd().DIRECTORY_SEPARATOR. 'comando.sh' ,'w+');
    fwrite($fp,  $bash);
    fclose($fp);
    shell_exec(bash.' && chmod a+x comando.sh');
}
